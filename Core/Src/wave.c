...
uint8_t WaveFileBuf[WAVEFILEBUFSIZE] __attribute__((aligned(8)));
uint8_t TempBuf[WAVETEMPBUFSIZE] __attribute__((aligned(8)));
...
uint8_t wave_decode_init(char* fname,wavctrl* wavx)
{
	...
	{
		CloseFileFlag=1;
		f_read(&WavFile,TempBuf,WAVEFILEBUFSIZE/2,(UINT*)&br);									//读取WAVEFILEBUFSIZE/2字节数据
